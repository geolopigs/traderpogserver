class FlyerInfosController < ApplicationController
  ...
  def getflyer(index)
    ...
    @flyer_info = FlyerInfo.find(index, :select => "id, topimg, sideimg, capacity, speed, stormresist, multiplier, price, tier")
  ...
  def index
    @flyer_infos = FlyerInfo.all(:select => "id, topimg, sideimg, capacity, speed, stormresist, multiplier, price, tier")
    @language = ApplicationHelper.preferred_language(request.headers["Accept-Language"])
